...
package com.metrostarsystems.ebriefing.BookPage.Contents.TabAnnotated;
...
public class FragmentAnnotated extends AbstractPagerFragment {
	
	private static final String TAG = FragmentAnnotated.class.getSimpleName();
	...
	public static final FragmentAnnotated newInstance() {
		FragmentAnnotated fragment = new FragmentAnnotated();
	...
	private void generateList() {
		...
		for(int index = 0; index < mSections.size(); index++) {
			...
			for(int pages = 0; pages < section.size(); pages++) {
				if(mApp.data().database().annotationsDatabase().has(mBook.id(), section.pageNumber(pages))) {
