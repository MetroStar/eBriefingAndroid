...
public class ServerInfo2010Task extends AbstractSyncTask<Void, Void, ServerInfoObject2> {
	
	private static final String TAG = ServerInfo2010Task.class.getSimpleName();
	
	private ServerInfo2010TaskListener		mListener;
	
	public ServerInfo2010Task(ServerInfo2010TaskListener listener, ServerConnection connection) {
		super(ServerInfo2010Task.class, SyncTaskType.TYPE_SERVER_INFO, connection);
		mListener = listener;
	}
	...
	protected void onPostExecute(ServerInfoObject2 result) {
		...
		mListener.onServerInfo2010TaskFinishedListener(result);
	}
	...
	protected ServerInfoObject2 executeRequest(AbstractSoapRequest request) {
		...
		if(SyncService.isNetworkAvailable()) {
			...
			try {
				response = new GetServerRequest(serverConnection(), ServerType.TYPE_SERVER_2010).execute(ServerInfo2010Task.class, request);
			} catch(Exception e) {
				e.printStackTrace();
			}
	...
	public static interface ServerInfo2010TaskListener {
		public abstract void onServerInfo2010TaskFinishedListener(ServerInfoObject2 result);
	}
}
