...
package com.metrostarsystems.ebriefing.BookPage.Contents.TabBookmarked;
...
public class FragmentBookmarked extends AbstractPagerFragment {
	...
	public static final FragmentBookmarked newInstance() {
		FragmentBookmarked fragment = new FragmentBookmarked();
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		for(int index = 0; index < chapters.size(); index++) {
			mSections.add(new Section("Chapter " + String.valueOf(index + 1) + ": " + chapters.get(index).title()));
		}
	...
	private void generateList() {
		...
		for(int index = 0; index < mSections.size(); index++) {
			...
			for(int pages = 0; pages < section.size(); pages++) {
				if(mApp.data().database().bookmarksDatabase().has(mBook.id(), section.pageNumber(pages))) {
